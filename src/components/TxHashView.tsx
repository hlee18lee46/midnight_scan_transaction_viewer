// src/components/IdentifierDeepView.tsx
...
type Hex = string;
type BlockLite = { hash: Hex; height: number };
type Block = {
  hash: Hex; height: number; timestamp: number;
  author?: Hex | null; protocolVersion?: number | null;
  parent?: BlockLite | null; transactions?: { hash: Hex }[];
};
type ContractAction = {
  __typename?: string; address: Hex; state: Hex; chainState: Hex;
  transaction: { hash: Hex };
};
type Tx = {
  hash: Hex; protocolVersion?: number | null; applyStage: string;
  identifiers: Hex[]; raw?: Hex | null; merkleTreeRoot?: Hex | null;
  block?: Block | null; contractActions?: ContractAction[];
};
...
function download(filename: string, contents: string, mime="text/plain"){ const blob=new Blob([contents],{type:mime}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url;a.download=filename;a.click();URL.revokeObjectURL(url); }
...
export default function IdentifierDeepView({
  initialId,
  followRelated = false, // <- stays false to “keep the transaction I search”
}: { initialId: string; followRelated?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setErr(null); setTxs(null);
        const seed = await qByIdentifier(initialId);
        if (!alive) return;
        if (!followRelated) {
          setTxs(seed); // <- just what the identifier returns
          return;
        }
        // (Optional path if you ever want to re-enable deep follow)
        ...
        if (!alive) return;
        setTxs(uniqueBy(lists.flat(), t => t.hash));
      } catch (e: any) {
        if (alive) setErr(String(e?.message || e));
      }
    })();
    return () => { alive = false; };
  }, [initialId, followRelated]);

  function uniqueBy<T>(arr: T[], key: (x:T)=>string) {
    const seen = new Set<string>(); return arr.filter(x => (seen.has(key(x)) ? false : (seen.add(key(x)), true)));
  }
        ...
        <li key={tx.hash} className="rounded-xl border p-4 bg-white shadow">
          ...
          {tx.raw && (
            <div className="mt-2 text-sm">
              ...
              <button onClick={()=>download(`${tx.hash}.raw.hex`, String(tx.raw))} className="ml-2 text-xs px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">Download .hex</button>
              <details className="mt-1">
                <summary className="cursor-pointer text-xs underline">peek</summary>
                <pre className="mt-1 p-2 bg-gray-50 rounded max-h-40 overflow-auto text-[11px] break-all">{String(tx.raw)}</pre>
              </details>
            </div>
          )}
            ...
            {tx.block ? (
              <>
                <div className="font-mono text-xs break-all">
                  #{tx.block.height} • {fmtLocal(tx.block.timestamp)} <span title={fmtUTC(tx.block.timestamp)}>({age(tx.block.timestamp)})</span>
                </div>
                <div className="font-mono text-xs break-all">
                  Block Hash: {tx.block.hash} <Copy text={tx.block.hash} />
                </div>
                {typeof tx.block.protocolVersion === "number" && <div className="text-xs">Block Protocol: {tx.block.protocolVersion}</div>}
                {tx.block.author && <div className="text-xs">Author: {tx.block.author} <Copy text={tx.block.author} /></div>}
          ...
          <div className="mt-3">
            <button onClick={()=>download(`${tx.hash}.json`, JSON.stringify(tx, null, 2), "application/json")} className="text-xs px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">Download JSON</button>
          </div>
        </li>
      ))}
    </ul>
